...
-- {{{ Number formatting, taken from https://forum.rainmeter.net/viewtopic.php?t=38190&start=20
function helpers.round(input, idp)
    return tonumber(string.format('%.' .. (idp or 0) .. 'f', input))
end

function helpers.format_number(num, idp)
    local rnum = tonumber(num)
    local scales = {'B', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB'}
    local scale = rnum == 0 and 1 or math.ceil(math.log(rnum) / (10 * math.log(2)))
    local scaled = helpers.round((rnum / 1024 ^ (scale - 1)), idp or 0)
